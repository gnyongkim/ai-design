...
### 5. Skeleton & Loading UI (스켈레톤 규칙)
- **명확한 색상 대비(Contrast)**: 화면 전체의 테마(Light/Dark)와 확연히 구분되도록 시맨틱 컬러 대신 고정 Neutral 팔레트를 크로스로 활용합니다.
  - 기본 레이아웃 카드/박스: `bg-neutral-200 dark:bg-neutral-800`
  - 텍스트 라인/아이콘 포인트: `bg-neutral-300 dark:bg-neutral-700`
- **애니메이션 및 형태**: `animate-pulse` 사용 시 애니메이션이 어색하지 않도록 화면 전환 시 렌더링 될 실제 컨텐츠 엘리먼트와 `border-radius`, 여백(Margin/Padding), 레이아웃을 100% 동일하게 모사하여 레이아웃 점프를 방지합니다.

### 6. 금지 패턴 (Anti-Pattern)
> [!CAUTION]
